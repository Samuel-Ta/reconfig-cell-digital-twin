"""
Step 1 bringup: UR5 + gripper in empty Gazebo Harmonic world.
Starts: Gazebo, robot_state_publisher, gz_ros2_control, MoveIt move_group, RViz.
Success test: command arm to 'home' named state; it moves in both Gazebo and RViz.
"""
...
def launch_setup(context, *args, **kwargs):
    ...
    moveit_config = (
        ...
        .robot_description_semantic(
            file_path="config/moveit/ur5_with_gripper.srdf"
        )
        .robot_description_kinematics(
            file_path="config/moveit/kinematics.yaml"
        )
        .joint_limits(
            file_path="config/moveit/joint_limits.yaml"
        )
        .planning_pipelines(
            pipelines=["ompl"],
        ...
        .trajectory_execution(
            file_path="config/moveit/moveit_controllers.yaml"
        )
        .to_moveit_configs()
    )

    # ── LD_PRELOAD shim for fastcdr missing serialize(uint) overload ───────────
    ...
    # Controllers are spawned AFTER the robot is in Gazebo (spawn_robot exits)
    jsb_spawner = Node(
    ...
    move_group_node = Node(
        ...
        parameters=[
            ...
            {
                "use_sim_time": True,
                # Warehouse config required to avoid null db-connection crash
    ...
    # ── sequencing ─────────────────────────────────────────────────────────────
    # 1. Give Gazebo 5 s to fully start, then spawn robot
    delayed_spawn = TimerAction(period=5.0, actions=[spawn_robot])

    # 2. After robot spawn exits (robot is now in Gazebo), spawn JSB after a
    #    small delay to let the hardware interfaces register
    ...
    # 3. Arm + gripper controllers after JSB is active
    ...
    # 4. MoveIt + RViz after arm controller is active
